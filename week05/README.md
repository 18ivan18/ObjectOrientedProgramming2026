...
Напишете клас `Rational`, представляващ рационално число като дроб (числител/знаменател) с цели числа.

**Изисквания:**
- Конструктор `Rational(int num, int den)` – знаменателят не трябва да е 0
- Всички резултати от аритметични операции да са **винаги опростени** (най-малка форма): напр. 1/4 + 1/4 = 2/4 → 1/2
- Поддръжка на смесени операции: `Rational + int`, `int + Rational`, и аналогично за `-`, `*`, `/`

**Оператори за аритметика:** `+`, `-`, `*`, `/`

**Оператори за сравнение:** `==`, `!=`, `<`, `<=`, `>`, `>=`

**Stream оператор:** `operator<<` – извеждане във вида `"num/den"` (напр. `3/4` или `-1/2`)

**Допълнително:** Конструктор с подразбиращ се знаменател 1 – `Rational(n)` е еквивалентно на `Rational(n, 1)`. Методи `getNumerator()` и `getDenominator()` за достъп до компонентите.

## Задача 5 (за вкъщи)

Надградете класа `Vector` от предишното упражнение (седмица 3) със следните допълнения:

**Специални методи:**
- Copy конструктор – deep copy на елементите
- Оператор за присвояване `operator=` – с проверка за самоприсвояване

**Предефинирани оператори:**
- `operator[]` – достъп до елемент по индекс (константен и неконстантен вариант)
- `operator==` и `operator!=` – сравнение по съдържание (два вектора са равни, ако имат еднакъв размер и еднакви елементи)
- `operator+` – конкатенация на два вектора (връща нов вектор)
- `operator+=` – добавяне на елементите на втория вектор към първия
- `operator<<` – извеждане във вида `[e0, e1, e2, ...]`
