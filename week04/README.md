...
Напишете методи `isEmpty()` – връща `true` ако URL и HTML съдържанието са празни, и `clear()` – изчиства таба (нулира URL и HTML, като обектът остава валиден).

Напишете метод `loadFrom(const BrowserTab &other)` – зарежда съдържанието на текущия таб от подадения като параметър (копира URL и HTML от `other` в текущия обект).

**Надграждане – клас Browser:** Създайте клас `Browser`, който е контейнер за `BrowserTab` обекти. Реализирайте методи за добавяне на таб, премахване на таб по индекс, достъп до таб по индекс, връщане на броя табове. Добавете индекса на активния таб и метод за превключване между табове. Не забравяйте за конструктор, деструктор и копиране.
...
**Разширение:** За `Point` – метод `midpoint(const Point &other)`, връщащ средата между текущата точка и подадената. За `Line` – метод `midpoint()` за средата на отсечката, `isParallelTo(const Line &other)` за проверка дали две линии са успоредни, `contains(const Point &p)` за проверка дали точка лежи на правата.
...
**Разширение:** Добавете методи `remainingActivations()`, `canActivate()`, `isExpired()` за състоянието на лиценза. Въведете enum `LicenceType` (PERSONAL=1, FAMILY=5, ENTERPRISE=неограничен). Добавете срок на годност, използвайки клас `Timestamp` – надградете го с методи за сравнение `isBefore()` и `isAfter()`. Метод `isExpiredByDate()` проверява дали лицензът е изтекъл по дата.
