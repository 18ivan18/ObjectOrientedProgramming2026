## Теория: Полиморфно копиране — `clone()`

### Проблемът — object slicing

Когато пазим обекти чрез указател към базов клас (`Base*`), обикновеното копиране не работи правилно. Ако просто копираме указателя, двата указателя сочат към един и същ обект. Ако се опитаме да копираме самия обект през `Base`, копираме само базовата му част и губим всичко, което е добавено в наследниците — явление, наречено **object slicing**.
...
void bad(const Base* src) {
    Base* copy = new Base(*src); // не се компилира: Base е абстрактен
                                 // а ако се компилираше — губим extra
}
```

### Решението — виртуален `clone()`

Добавяме чисто виртуален метод `clone()` в базовия клас. Всеки наследник го предефинира и връща `new Derived(*this)` — дълбоко копие на реалния тип.
...
void good(const Base* src) {
    Base* copy = src->clone(); // правилен тип, правилни данни
    // ...
    delete copy;
}
```

Шаблонът е задължителен при всеки клас, чиито обекти се пазят в контейнер от `Base*` и трябва да се копират коректно (например масив от превозни средства в гараж).
